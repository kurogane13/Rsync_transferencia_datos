...
do
  ...
  "- Ver scripts")
	 ...
	 echo "$(date) ERROR: No hay scripts con esa descripcion en la carpeta scripts_conexiones" | tee -a syslog.log
	 echo " "
	 echo "$(date) NOTIFICACION - MENSAJE: Fin modo visualizacion" | tee -a syslog.log
     echo "----------------------------------------------------------------------"
	 echo "$(date) NOTIFICACION - MENSAJE: Volviendo a menu principal"  | tee -a syslog.log
  ...
  "+ Crear nuevo archivo de transferencia")
     ...
     echo " " >> scripts_conexiones/$archivo$archivo_exclusion_datos
	 echo "# Este script excluira los datos descritos en el archivo de texto: $archivo$txt_exclusion" >> scripts_conexiones/$archivo$archivo_exclusion_datos
     echo " " >> scripts_conexiones/$archivo$archivo_exclusion_datos
	 ...
	 echo "mypwd="${PWD}/ >> scripts_conexiones/$archivo$archivo_exclusion_datos
     ...
     echo " "
     echo 'sshpass -p "$clave" rsync -av --ignore-existing --ignore-missing-args --stats --update --progress --out-format="%t %f" $pathorigen --exclude-from=$mypwd$scripts_conexiones$archivo$archivo_carpetas_exclusion ssh $usuario@$servidor:$pathdestino' >> scripts_conexiones/$archivo$archivo_exclusion_datos
