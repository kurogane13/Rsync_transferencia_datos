...
do
  ...
  "+ Crear nuevo script de transferencia hacia Servidor Remoto")
     ...
     echo " "
     echo 'sshpass -p "$clave" rsync -av --ignore-existing --ignore-missing-args --stats --update --progress --out-format="%t %f" $pathorigen --exclude-from=$mypwd$scripts_conexiones_exclusion_datos$archivo$archivo_carpetas_exclusion ssh $usuario@$servidor:$pathdestino' >> scripts_conexiones_exclusion_datos/$archivo$archivo_exclusion_datos
     ...
     echo " " >> scripts_conexiones_exclusion_datos/$archivo.sh
     echo 'sshpass -p "$clave" rsync -av --ignore-existing --stats --update --progress --out-format="%t %f" $pathorigen  ssh $usuario@$servidor:$pathdestino' >> scripts_conexiones_exclusion_datos/$archivo.sh
     echo " "
     echo "$(date) NOTIFICACION - MENSAJE: El script se ha generado dentro de la carpeta: scripts_conexiones_exclusion_datos" | tee -a syslog.log
     echo " "
  ...
  "+ Crear nuevo script local de transferencia hacia directorio/disco ")
	 ...
	 echo " " >> $mypwd$Dir/$nombre_script$script_exclusion_datos_local
	 echo "rsync -av --ignore-existing --stats --update --progress --out-format='%t %f' $path_de_origen --exclude-from=$mypwd$Dir/$nombre_script$archivo_exclusion_datos_local $path_de_destino" >> $mypwd$Dir/$nombre_script$script_exclusion_datos_local
	 echo " "
	 ...
	 echo " " >> $mypwd$Dir/$nombre_script$script_sin_exclusion_datos_local
	 echo "rsync -av --ignore-existing --stats --update --progress --out-format='%t %f' $path_de_origen $path_de_destino" >> $mypwd$Dir/$nombre_script$script_sin_exclusion_datos_local
	 echo " "
	 echo "$(date) NOTIFICACION - MENSAJE: Volviendo a menu principal" | tee -a syslog.log
	 echo " "
	 read -p "Presione enter para desplegar el menu... " enter
